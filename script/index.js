document.addEventListener('scroll', () => {
    if (window.innerWidth > 960) {
        document.querySelector('#top').style.backgroundColor = window.scrollY > 0 ? "rgba(8, 10, 20,0.9)" : "rgba(0,0,0,0)"
    }
})

const scrollToMain = () => {
    window.scrollTo({
        top: document.getElementById("main").getBoundingClientRect().bottom,
        behavior: "smooth"
    })
}
...
const createPlatformImage = (src) => {
    const img = document.createElement("img")
    img.src = src
    return img
}

const setPlatformSources = (desktop) => {
    // the assets loaded on mobile are about twice as big but still look sharp on small screens.
    const ghSrc = `/assets/github${desktop ? "" : "-m"}.png`
    const mnSrc = `/assets/manual${desktop ? "" : "-m"}.png`
    for (const it of document.getElementsByClassName("gh_img")) {
        it.src = ghSrc
    }

    for (const it of document.getElementsByClassName("mn_img")) {
        it.src = mnSrc
    }

    // request might start before src is changed leading to unnecessary loss, so I create the nodes when needed
    document.getElementById("viperh")?.prepend(createPlatformImage(`/assets/viper${desktop ? "" : "-m"}.png`))
    document.getElementById("vtolh")?.prepend(createPlatformImage(`/assets/vtol${desktop ? "" : "-m"}.png`));

    document.getElementById("browser-globe").src = `/assets/globe${desktop ? "" : "-m"}.png`

    if (desktop) {
        // video is lagging on mobile and 8mb is quite large so this is desktop only
        document.getElementById("lazyvideo").src = "/assets/NorthstarPromoVid.mp4"
    }
}

// images set by this won't load on browser with javascript disabled
setPlatformSources(!navigator.userAgent.match(/iphone|android/ig))
